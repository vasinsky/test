<?php
  class EditgaleryController extends BaseController{
      ...
      public function getGaleryData($glid){
         $result = $this->model->getGaleryData($glid);
         
         if($result['success'] === false){
             Bufer::set(array('errors'=>array('error')));
             return false;
         }
         else{
            Bufer::add(array('data'=>$result['data']));
            return $result['data'];
         }
      ...
      public function renderForm($data){
          ...
          $form->setInput('name_galery')
               ...
               ->setAttr('value|'.(isset($_POST['name_galery']) ? $_POST['name_galery'] : $data['meta']['gname']))
          ...
          $form->setInput('sinonim_galery')
               ...
               ->setAttr('id|child')
               ->setAttr('class|form-control input-lg')
               ->setAttr('value|'.(isset($_POST['sinonim_galery']) ? $_POST['sinonim_galery'] : $data['meta']['gindex']))
          ...
          $form->setTextarea('gdescription')
               ...
               ->setText((isset($_POST['gdescription']) ? $_POST['gdescription'] : $data['meta']['gdescription']))
               ->setRules('notempty|trim')
          ...
          $form->setInput('savegalery')
               ->setAttr('type|submit')
               ->setAttr('class|btn btn-primary btn-lg')
               ->setAttr('value|Сохранить галерею')
               ->addInput();  
          ...
          if(!$form->sendForm('savegalery')){
              ...
              Bufer::add(array('errors'=>$errors));
                 ...
                 $data = array(
                             ...
                             'image'=>isset($_POST['cover']) ? $_POST['cover'] : false,
                             'glid' => $glid         
                 );   
                 ...
                 if(isset($_POST['giname']) && !empty($_POST['giname'])){
                     foreach($_POST['giname'] as $giid=>$v){
                        if($v != ' '){
                            $meta_images[$giid] = array(
                                   'giname'=>$this->model->escape($v),
                                   'gidescription'=>$this->model->escape($_POST['gidescription'][$giid])
                            );
                        }
                     }
                 }         
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
     public function EditGaleryFromDb($data){
         ...
         $result = $this->model->EditGaleryFromDb($data);
         
         //Запись в БД прошла
         if($result['success'] === true){
           Route::go('?mode=admin&route=editgalery&glid='.$data['glid']);              
         }
         else{
  ...
  if($getData !== false){
      $controller->renderForm($getData);
  }
  
  if(isset($_GET['delete_giid'])){
     $giid = (int)$_GET['delete_giid'];
     
     $controller->deleteImage($giid);
  }
  
  $controller->view(ADMIN_TPLS_DIR.'/header.tpl');
  $controller->view(ADMIN_TPLS_DIR.'/editgalery.tpl');
