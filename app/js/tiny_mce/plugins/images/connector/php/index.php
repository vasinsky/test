...
class TinyImageManager {
	...
	function TinyImageManager() {
        ...
        $this->ALLOWED_MULTI = array('jpeg','jpg','gif','png','doc','docx','ppt','pptx','xls','xlsx','mdb','accdb', 'swf', 'zip', 'rar', 'rtf', 'pdf', 'psd', 'mp3', 'wma');
		...
		$this->dir = array(
            ...
            'multi'		=> realpath(DIR_ROOT.DIR_MULTI)
		);
		...
		switch ($_POST['action']) {
					...
					if (preg_match('/[a-z0-9-_]+/sim', $_POST['name'])) {
						if(is_dir($dir.'/'.$_POST['name'])) {
							...
							if(mkdir($dir.'/'.$_POST['name'])) {
                                ...
                                $result['tree'] .= $this->DirStructure('multi', 'first', $dir.'/'.$_POST['name']);
			...
			case 'showtree':
				...
				if($_POST['type']=='multi') $this->firstAct = true;
				if($_POST['type']=='multi') echo $this->DirStructure('multi', 'first');
				else 					
	...
	function AccessDir($requestDirectory, $typeDirectory) {
		...
		} elseif($typeDirectory == 'multi') {
			$full_request_files_dir = realpath($this->dir['multi'].$requestDirectory);
			if(strpos($full_request_files_dir, $this->dir['multi']) === 0){
				return $full_request_files_dir;
			} else return false;
        } else return false;
	...
	function Tree($beginFolder) {
		...
		if ($handle) {
			$struct[$beginFolder]['path'] = str_replace(array($this->dir['multi'], $this->dir['files'], $this->dir['images']),'',$beginFolder);
	...
	function DirStructure($type, $innerDirs='first', $currentDir='', $level=0) {
		...
		if($innerDirs == 'first') {
			...
			foreach ($innerDirs as $v) {
			    if($type == 'images') $nf = 'Изображения';
                elseif($type=='files') $nf= 'Файлы';
                elseif($type=='multi') $nf= 'Multi';
             
				$ret = '<div class="folder'.ucfirst($type).' '
                .$firstAct.'" path="" pathtype="'.$type.'">'
                .$nf.($v['count']>0?' ('.$v['count'].')':'')
	...
	function UploadFile($dir, $type) {
		...
		if(isset($_POST['Filename'])) {
            ...
            elseif($pathtype == 'multi') $allowed = $this->ALLOWED_MULTI;
	...
	function ShowDir($dir, $type) {
		...
		foreach ($dir as $v) {
			...
			if($v['general']['width'] > WIDTH_TO_LINK || $v['general']['height'] > HEIGHT_TO_LINK) {
				...
				list($middle_width, $middle_height) = getimagesize(DIR_ROOT.'/'.PATH.$middle_thumb);
            	$middle_thumb_attr = 'fmiddle="'.DIR_ROOT.'/'.rtrim(PATH,'/').$middle_thumb.'" fmiddlewidth="'.$middle_width.'" fmiddleheight="'.$middle_height.'" fclass="'.CLASS_LINK.'" frel="'.REL_LINK.'"';
			} else {
				$middle_thumb = '';
				$middle_thumb_attr = '';
			}
            ...
            //$thumb = DIR_ROOT.$thumb;

   $thumb = DIR_ROOT.'/'.PATH.ltrim($thumb,'/');
			$ret .= '
   <table class="imageBlock0" cellpadding="0" cellspacing="0" filename="'.$v['general']['filename'].'" 
   fname="'.$v['general']['name'].'" 
   type="'.$type.'" 
   ext="'.strtoupper($v['general']['ext']).'" 
   path="'.rtrim(PATH,'/').$v['general']['path'].'" 
   linkto="'.rtrim(PATH,'/').$v['general']['link'].'" 
   fsize="'.$v['general']['size'].'" 
   fsizetext="'.$this->bytes_to_str($v['general']['size']).'" 
   date="'.date('d.m.Y H:i',$v['general']['date']).'" 
   fwidth="'.$v['general']['width'].'" 
   ...
   md5="'.$v['general']['md5'].'" '.$middle_thumb_attr.'><tr><td valign="bottom" align="center">
	...
	function GetThumb($dir, $md5, $filename, $mode, $width=100, $height=100) {
       $filename = DIR_ROOT.'/'.ltrim(DIR_IMAGES,'//').'/'.$filename;
